...
## Первым делом — полный gate на новом pin

ИСПРАВЛЕНО ПОСЛЕ ТРЕТЬЕЙ G5. Этот раздел прежде утверждал, что второй отчёт не
назвал результат обязательного полного `tools.check --deliver` на pin 4dad08e.
Утверждение неверно: отчёт назвал его разделом «Verdict — полный обязательный
deliver gate: PASS», exit 0, 251 passed, 11 contracts kept, два committed raw в
коммите 31e6a47. Ошибка автора. Заголовок раздела и три строки заменены этой
поправкой. Тот же раздел ниже получил вторую правку: из него убрана неверная
оговорка про неполный gate. Больше в нём ничего не переписано.
Прогон gate заново на новом pin остаётся обязательным потому, что pin другой.
Выполни его целиком, не отдельные инструменты, и назови результат явно. Точные команды — REPRODUCE.md
...
source: фактическое новое число бери из вывода. Максимум
три повтора одного gate. Затем `tools.probe_overview`.

## Проверить ровно оставшееся

1. Изучи исходную дельту `git diff 8b07b7a 456776a`: ровно четыре файла —
   ...
   Диапазон 4dad08e → 456776a дополнительно содержит промежуточные docs-only
   commits 6fdfbea и 8b07b7a, это ожидаемо.
