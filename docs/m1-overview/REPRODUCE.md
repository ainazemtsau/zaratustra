...
проверок. Прежние 235 tests относятся к T5: фактическое новое число берётся из
вывода, а не подставляется. Максимум три повтора одного failed gate. Не исправлять
...
из производного представления. Пустой Core diff — идентичность байтов, а не
поведенческий PASS. Все три done_when проверяются отдельно; обе гарантии
