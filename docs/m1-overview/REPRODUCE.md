...
проверок. Прежние числа тестов относятся к прежним source и к неполному gate:
фактическое новое число берётся из вывода, а не подставляется. Максимум три повтора одного failed gate. Не исправлять
