...
операцией. `basis = null` означает только одно: основание снято санацией, потому что
удалён Artifact, названный закрытой ревизией (раздел «Обслуживание данных»).
...
- Удаление Artifact санирует зависимые основания закрытий тем же механизмом адресов
  зависимостей (`sanitize_deleted_dependency`), что основания подтверждений прохода 1.
  - **Зависимость.** Основание зависит от удалённого Artifact, если его адрес записан в
    закрытой ревизии Work: во входах, связанных выходах или предпосылках `stale`. Это
    те же адреса, которыми проход 1 описывает зависимости состояния Work.
  - **Что остаётся.** Исход, адреса и прочее состояние Work сохраняются, производное
    состояние не меняется. В ревизии остаётся `basis = null`, удалённое основание не
    выдаётся как оригинал.
  - **Повтор и backups.** Квитанция закрытия выходит из повтора (`history_unavailable`,
    `read_receipt` — `not_found`). Backups, содержащие этот Work, помечаются
    загрязнёнными и очищаются `complete_deletions`.
  - **Независимые закрытия.** Закрытия, не называющие удалённый Artifact, сохраняют
    основание и повтор.
- После удаления такого Artifact его адрес читается в `unavailable_refs` тем же
...
документы — `e2d5b90`; исправления по review — `210399b`. Проверки ниже запускались на
чистом дереве `210399b`. Итоговый коммит с этим документом меняет только Markdown-файлы.
Среди них `src/zaratustra/foundation/AGENTS.md`, который входит в wheel, поэтому SHA-256
wheel итогового коммита отличается от проверенного; Python-код и тесты совпадают.
...
- Воспроизведение review на `e2d5b90` и после исправления — один и тот же сценарий
  через публичный Core (скрипт вне репозитория).
  - На `e2d5b90`: маркер оставался в `closure.basis`, закрытой SQLite, новом backup и
    после перезапуска; повтор возвращал квитанцию. `stale` по `X@999` был записан и
    включил удержание: назначение `stop_requested`, вопрос закрыт. `stale` по
    несозданному `D@1` тоже был записан.
  - После исправления: маркера нигде нет, повтор — `history_unavailable`, оба `stale` —
    `premise_unknown`, ребёнок остаётся `proposed`/`waiting`.
- Новые регрессии до исправления: два теста дефектов упали по тем же причинам, а
  положительный случай удалённой известной предпосылки прошёл.
- `pytest tests -q` на `210399b`, conda-forge Python 3.13.7 с SQLite 3.53.3 и FTS5:
  564 прошли, 1 упал. Это 562 теста `e2d5b90` и 3 новых. Единственное падение —
...
- `tools.probe_stage6_rpc` (checkout), `tools.probe_install_stage6` (wheel SHA-256
  `5AE5D2AD6740C92191DD7199521CBB44B9ACCDDE38C13453BEF500E870087D1E` в новом venv вне
  ...
  - Отчёты — `_scratch/stage6-pass3-31fix-rpc`, `_scratch/stage6-pass3-31fix-installed`,
    `_scratch/stage6-pass3-31fix-stage5` (игнорируются git).
...
uv run --locked python -m tools.probe_stage6_rpc --output _scratch\stage6-pass3-31fix-rpc --pi-runtime '_scratch\stage4-pi-runtime'
uv run --locked python -m tools.probe_install_stage6 --output _scratch\stage6-pass3-31fix-installed --pi-runtime '_scratch\stage4-pi-runtime' --sqlite-dll $env:ZARATUSTRA_SQLITE_DLL
```
...
- Основание закрытия ребёнка не снимается при удалении Artifact, который есть только в
  плане родителя (основание плана, входы родителя) и не назван в закрытой ревизии
  ребёнка. Так же в проходе 1 ведёт себя принятие ребёнка. Если такое основание
  считается зависимым, нужно решение владельца.
- Основание принятия (`WorkAcceptance.basis`, Stage 3) при удалении входа или выхода не
  снимается; ревизия лишь помечает адрес недоступным. Это принятое поведение Stage 3.
  Санация закрытий 3.1 его не меняет, но этот вопрос тоже вынесен владельцу.
