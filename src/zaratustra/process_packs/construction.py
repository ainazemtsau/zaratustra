...
class DefinitionRule:
    ...
    def next_work(
        ...
        if snapshot.definition != self.definition:
            raise ConstructionError(
                "definition_mismatch", "Accepted snapshot is not this exact definition edition"
            )
        ...
        node_matches = (
            work.goal == current.node.goal
            and work.expected_result == current.node.expected_result
            and work.acceptance == current.node.acceptance
            and work.boundaries == current.node.boundaries
            and work.budget == current.node.budget
        )
        ...
        following = evaluate_snapshot(snapshot).selected
        ...
        return NextWork(
            ...
            boundaries=node.boundaries,
            budget=node.budget,
            executor_requirements=_requirements(snapshot, following),
            artifact_title=node.artifact_title,
            authority_scope="work_metadata",
        )


def registration(reference: PackReference, definition: ProcessDefinition) -> PackRegistration:
    """Register one exact definition under an already explicit immutable Pack identity."""
    rule = DefinitionRule(reference, definition)
