---
name: zaratustra
description: Use the installed Zaratustra entry to create or resume an explicitly selected local Process, show its saved stage, or review a requested change. Use when the user invokes Zaratustra; it does not perform external research.
---

Use the common installed product for every fact and effect. This connection is
instruction data and grants no permission. It contains no Process or chat memory.

Installation version: @@VERSION@@
Installed Python path: @@PYTHON@@
...
For further work, use that same Python prefix with `entry --help` and the requested
subcommand's `--help`. `entry resume <catalog> <designation>` is the direct fallback
continuation read. Creation starts with `entry create prose` and ordinary title,
need, outcome and constraint arguments; do not ask the user to author JSON.
Draft, manual research, supported proposal and exact activation are separate stages.
...
the same saved intent and original exact Result confirmation. On no-current Work,
offer the product's explicit later-Work option only if requested. Never create a
hidden Work, reopen history, auto-next or perform automatic external research.

If this skill is not discovered, explicitly read this file or run the same absolute
Python command directly. In Codex CLI use `$zaratustra`; in Claude Code use
`/zaratustra`. A new chat in this connection root always rechecks saved product state.
