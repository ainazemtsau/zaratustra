...
Windows-проверку владельца на `d33b676` (`tools.check --deliver`, 551 тест, оба исходных
Pi-probe, включая установленный wheel) эта сессия не наблюдала: результат передан с его
слов.
...
Windows-проверки `d33b676` передан со слов владельца. Тесты не читают документы
репозитория, поэтому изменения документов на pytest не влияют.
